...
export async function createUser(email, password, username) {
    try {
      ...
      // Create a user document in the database with the required fields
      const newUser = await databases.createDocument(
        config.databaseID,           // Database ID
        config.usersCollectionID,    // Users collection ID
        ID.unique(),                 // Unique document ID
        {
          accountID: newAccount.$id, // Required field: Pass the account ID
          email: email,
          userName: username,        // Adjust field name to match collection schema
          avatar: avatarUrl.href,    // Optional: Avatar URL
        }
      );
